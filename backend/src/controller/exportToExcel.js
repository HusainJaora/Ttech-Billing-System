...
// Helper function to add Sr.No. column
const addSerialNumberColumn = (worksheet, data) => {
...
const exportSuppliers = async (req, res) => {
  const { signup_id } = req.user;
  const { q, from_date, to_date } = req.body;
  ...
  try {
    ...
    // Define columns (Sr.No. will be added by helper)
    worksheet.columns = [
      { header: "Supplier ID", key: "supplier_id", width: 20 },
      { header: "Supplier Legal Name", key: "supplier_Legal_name", width: 40 },
      { header: "Created Date", key: "created_date", width: 20 },
...
const exportProductCategory = async (req, res) => {
  ...
  try {
    ...
    ProductCategory.forEach((row, index) => {
      worksheet.addRow({
        sr_no: index + 1,
        ...row
      });
