...
import { 
  ...
  Save, X, Sparkles, Clock, Calendar, AlertTriangle 
} from 'lucide-react';

// Invoice Terms & Conditions Page Component
export const InvoiceTermsConditions = () => {
  ...
  const fetchTerms = async () => {
    try {
      ...
      const response = await axiosInstance.get(ENDPOINTS.INVOICE_TERMS_CONDITIONS.GET_TERM);
  ...
  const handleAddTerms = async () => {
    ...
    try {
      const response = await axiosInstance.post(
        ENDPOINTS.INVOICE_TERMS_CONDITIONS.ADD_TERM,
        { terms: validTerms }
      );
  ...
  const handleUpdateTerm = async (termId) => {
    ...
    try {
      const response = await axiosInstance.put(
        `${ENDPOINTS.INVOICE_TERMS_CONDITIONS.UPDATE_TERM}/${termId}`,
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await axiosInstance.delete(
        `${ENDPOINTS.INVOICE_TERMS_CONDITIONS.DELETE_TERM}/${termToDelete.terms_id}`
      );
            ...
            <div className="flex items-center space-x-4 ml-12 lg:ml-0">
              ...
              <div>
                <h1 className="text-3xl font-bold bg-gradient-to-r from-gray-900 to-indigo-600 bg-clip-text text-transparent">
                  Invoice Terms & Conditions List
                </h1>
                <p className="text-sm text-gray-600 flex items-center mt-1">
                  <Sparkles className="h-3 w-3 mr-1 text-indigo-500" />
                  Manage your invoice terms (Max: 4 terms)
                </p>
              </div>
            </div>
            {terms.length < 4 && (
